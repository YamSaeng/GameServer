		...
		case en_GameObjectJobType::GAMEOBJECT_JOB_TYPE_COMBO_ATTACK_OFF:
			{
				...
				{
					Player->_ComboSkill->_ComboSkillTick = 0;
				}
			}
			break;
...
void CGameObject::ExperienceCalculate(CPlayer* TargetPlayer, CGameObject* TargetObject)
{
	...
	wstring ExperienceGetString;

	wchar_t ExperienceGetMessage[64] = L"0";
	...
	case en_GameObjectType::OBJECT_SLIME:
		...
		wsprintf(ExperienceGetMessage, L"슬라임에게 %d 만큼의 경험치를 얻었습니다.", TargetMonster->_GetExpPoint);
		
		ExperienceGetString = ExperienceGetMessage;
	...
	// 경험치 획득 시스템 메세지 전송
	CMessage* ResSkillSystemMessagePacket = G_ObjectManager->GameServer->MakePacketResChattingBoxMessage(TargetPlayer->_GameObjectInfo.ObjectId,
		en_MessageType::SYSTEM,
		st_Color::SkyBlue(),
		ExperienceGetString);
	G_ObjectManager->GameServer->SendPacket(TargetPlayer->_SessionId, ResSkillSystemMessagePacket);
	ResSkillSystemMessagePacket->Free();
}
