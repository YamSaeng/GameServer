...
public:	
	...
	//-----------------------------------------------------------------------------
	bool CheckEmptySpace(int16 PositionX, int16 PositionY, int32 Width, int32 Height);
	...
	// 매개변수로 받은 위치 아이템 초기화
	//----------------------------------------------------
	void InitItem(int16 TilePositionX, int16 TilePositionY);
	...
	//------------------------------------------------------------------------------------------------------
	bool OverlapCheck(int16 TilePositionX, int16 TilePositionY, int16 Width, int16 Height, CItem** OverlapItem);
	...
	//----------------------------------------------------------------------
	// 아이템이 속한 위치 값을 받아 아이템 정보 반환
	//----------------------------------------------------------------------
	CItem* FindInventoryItem(Vector2Int ItemPosition);	
	...
	// 가방에 있는 아이템들을 반환
	//---------------------------------------------------------------------------------
	vector<st_ItemInfo> FindAllInventoryItem();

	//-----------------------------------------------------------
	// 서버에 저장하기 위해 가방의 모든 칸 정보를 반환한다.
	//-----------------------------------------------------------
...
private:
	...
	// 보유중 아이템 목록 위치
	vector<Vector2Int> _ItemPositions;
};
