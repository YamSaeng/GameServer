...
#define ENDIAN_REVERSE_ULONG(dwS)	( (ROTL_ULONG((dwS),  8) & 0x00ff00ff)	\
	...
	#define BIG_D2B(D, B)		*(unsigned long*)(B) = (ULONG)(D)
	...
	#define LITTLE_D2B(D, B)	*(unsigned long*)(B) = (ULONG)(D)
#else
...
{
	unsigned int remain_buffer = Info->remain_num;
	...
	{
		memcpy((unsigned char*)(Info->szBuffer + remain_buffer), pszMessage, (signed int)SHA256_DIGEST_BLOCKLEN);
	...
	memcpy((unsigned char*)(Info->szBuffer + remain_buffer), pszMessage, uDataLen);
...
// o 출력				:
//*********************************************************************************************************************************
void SHA256_Close( OUT SHA256_INFO *Info, IN unsigned char*pszDigest )
{
	...
	if (Index > SHA256_DIGEST_BLOCKLEN - 8)
	{
		...
		memset((unsigned char*)Info->szBuffer + Index, 0, (signed int)(SHA256_DIGEST_BLOCKLEN - Index - 8));
