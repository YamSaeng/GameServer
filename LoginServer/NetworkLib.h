...
private:
	...
	//---------------------------------------------------------
	void ReleaseSession(st_LoginSession* ReleaseSession);	

	//------------------------------------
	// WSASend를 예약한다.
	//------------------------------------
	void SendPost(st_LoginSession* SendSession);

	//-------------------------------------------------------------------
	// WSARecv를 예약한다.
	//-------------------------------------------------------------------
	void RecvPost(st_LoginSession* RecvSession, bool IsAcceptRecvPost = false);

	//--------------------------------------------------------------------------------
	// Recv완료시 호출하는 함수
	//--------------------------------------------------------------------------------
	void RecvNotifyComplete(st_LoginSession* RecvCompleteSession, const DWORD& Transferred);
	//--------------------------------------------------------------------------------
	// Send완료시 호출하는 함수
	//--------------------------------------------------------------------------------
	void SendNotifyComplete(st_LoginSession* SendCompleteSession);
	...
	//-------------------------------------------------
	virtual void OnClientLeave(st_LoginSession *LeaveSession) = 0;
...
protected:
	...
	// 세션 포인터 배열
	//--------------------------------------------
	st_LoginSession* _SessionArray[SERVER_SESSION_MAX];
	...
	//-----------------------------------------
	st_LoginSession* FindSession(__int64 SessionID);

	//---------------------------------------------------------------------------------------------------------------
	// 증가시켜준 IOCount를 감소시켜주는 함수 ( 사용한다는 의미인 IOCount를 1 줄여줌으로써 반납 한다는 의미를 갖는다)
	//---------------------------------------------------------------------------------------------------------------
	void ReturnSession(st_LoginSession* Session);
public:
	//-------------------------
	// 연결되어 있는 세션 개수
	//-------------------------
