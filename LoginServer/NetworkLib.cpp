...
CNetworkLib::CNetworkLib()
{
	...
	{
		_SessionArray[SessionCount] = new st_LoginSession;
...
st_LoginSession* CNetworkLib::FindSession(__int64 SessionID)
{
...
void CNetworkLib::ReturnSession(st_LoginSession* Session)
{
...
void CNetworkLib::SendPacket(__int64 SessionID, CMessage* Packet)
{
	...
	// 함수 나가기전에 감소시켜준다.
	st_LoginSession* SendSession = FindSession(SessionID);
...
void CNetworkLib::SendPost(st_LoginSession* SendSession)
{
		...
		*/
		if (InterlockedExchange(&SendSession->IsSend, SENDING_DO) == SENDING_DO_NOT)
		{
			...
			/*
				...
				다시 한번 데이터가 들어 있는지 확인하는 이유는 다음과 같다.
...
void CNetworkLib::RecvPost(st_LoginSession* RecvSession, bool IsAcceptRecvPost)
{
	int RecvBufCount = 0;
	WSABUF RecvBuf[2];
...
void CNetworkLib::RecvNotifyComplete(st_LoginSession* RecvCompleteSession, const DWORD& Transferred)
{
	// ---------------------------------------------------------------------------------------
...
void CNetworkLib::SendNotifyComplete(st_LoginSession* SendCompleteSession)
{
	//-------------------------------------------------------------
	//	완료 통지된 패킷 정리
	//	해당 세션이 보낸 패킷의 개수만큼
...
{
	st_LoginSession* DisconnectSession = FindSession(SessionID);
...
unsigned __stdcall CNetworkLib::WorkerThreadProc(void* Argument)
{
	...
	if (Instance)
	{
		st_LoginSession* NotifyCompleteSession = nullptr;
		for (;;)
		{
			DWORD Transferred = 0;
...
unsigned __stdcall CNetworkLib::AcceptThreadProc(void* Argument)
{
	...
	if (Instance)
	{
		for (;;)
		{
			...
			int nValue = 1;			
			int RetValue = setsockopt(NewSession->ClientSock, IPPROTO_TCP, TCP_NODELAY, (const char*)&nValue, sizeof(nValue));
...
void CNetworkLib::ReleaseSession(st_LoginSession* ReleaseSession)
{
